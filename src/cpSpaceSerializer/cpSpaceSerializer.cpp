...
bool elementEquals(TiXmlNode *elm, const char* value)
{
	return stringEquals(elm->Value(), value);
}

template <typename T>
T stringToValue(const char* str)
{
	T value(0);
...
template <typename T>
T getAttribute(TiXmlElement *elm, const char* name)
{
	return stringToValue<T>(elm->Attribute(name));
}

template <typename T>
T createValue(const char* name, TiXmlElement* elm)
{
	T value(0);
...
template <typename T>
void setAttribute(TiXmlElement *elm, const char* name, T value)
{
...
template <typename T>
TiXmlElement *createValueElm(const char* name, T value)
{
...
cpSpace* cpSpaceSerializer::load(cpSpace *space, const char* filename)
{
	...
	space->elasticIterations = createValue<int>("elasticIterations", root);
...
bool cpSpaceSerializer::save(cpSpace* space, const char* filename)
{
	...
	root->LinkEndChild(createValueElm("elasticIterations", space->elasticIterations));
	...
	writeShapeSet(root, space->activeShapes->handleSet);
	writeShapeSet(root, space->staticShapes->handleSet);

	//Write out constraints
	for(int i=0; i<space->constraints->num; i++)
	{
		cpConstraint *constraint = (cpConstraint *)space->constraints->arr[i];
		root->LinkEndChild(createConstraintElm(constraint));
	}
	
	return _doc.SaveFile(filename);
}

void cpSpaceSerializer::writeShapeSet(TiXmlNode* root, cpHashSet *set)
{
	for(int i=0; i<set->size; i++)
	{
		...
		while(bin)
		{
			...
			root->LinkEndChild(createShapeElm(shape));			
